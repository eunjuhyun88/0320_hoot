...
function autopilotConfig(config) {
  return {
    ...
    autoCommit: config.autopilot?.autoCommit ?? false,
    autoCommitStages: config.autopilot?.autoCommitStages ?? ['stop'],
...
if (autopilot.autoCommit && autopilot.autoCommitStages.includes(stage)) {
  run('bash', ['scripts/dev/auto-commit-context.sh', '--stage', stage, '--work-id', workId]);
}
