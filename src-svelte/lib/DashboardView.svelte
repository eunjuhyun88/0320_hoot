<script lang="ts">
  ...
  import GlobeCanvasIsland from "./GlobeCanvasIsland.svelte";
  ...
  onMount(() => {
    ...
    // Set dark theme on body for dashboard
    ...
    return () => {
      ...
      if (joinDeltaTimeout !== null) window.clearTimeout(joinDeltaTimeout);
      liveCleanup?.();
      // Restore body styles for landing page
...
<div class="viewport" class:mounted data-theme="dark">
    ...
    <header class="island">
      <div class="island-left">
        <div class="island-brand">
          ...
          <button class="brand-name" on:click={() => router.navigate('landing')}>HOOT</button>
          {#if !isMobile}
...
<style>
  ...
  .island {
    ...
    width: calc(100% - 24px);
    border-radius: 100px;
    background: var(--surface-elevated, rgba(12, 12, 16, 0.75));
  ...
  @keyframes pulse-dot {
    0%, 100% { opacity: 1; box-shadow: 0 0 12px rgba(217, 119, 87, 0.5); }
    50% { opacity: 0.7; box-shadow: 0 0 6px rgba(217, 119, 87, 0.3); }
  }
  ...
  .status-streaming { color: var(--globe-mint, #2ad47d); }
  .status-connecting { color: var(--globe-orange, #ffb44d); }
  .status-error { color: var(--globe-pink, #ff5d73); }
  .status-offline, .status-fixture { color: var(--text-muted, rgba(255, 255, 255, 0.4)); }
  ...
  .stat-card {
    ...
    background: var(--surface, rgba(8, 10, 16, 0.65));
  ...
  .stat-card:hover {
    background: var(--surface-elevated, rgba(12, 14, 20, 0.75));
  ...
  .stat-value.accent-cyan { color: var(--globe-cyan, #33d1ff); }
  .stat-value.accent-amber { color: var(--globe-orange, #ffb44d); }
  ...
  .tc-inline.keep { background: rgba(42, 212, 125, 0.12); color: var(--globe-mint, #2ad47d); }
  .tc-inline.discard { background: rgba(255, 93, 115, 0.12); color: var(--globe-pink, #ff5d73); }
  .tc-inline.crash { background: rgba(216, 76, 255, 0.12); color: var(--globe-magenta, #d84cff); }
  ...
  .hud-right {
    ...
    flex-direction: column;
    border-radius: 14px;
    background: var(--surface, rgba(8, 10, 16, 0.68));
    border: 1px solid var(--border, rgba(255, 255, 255, 0.04));
    backdrop-filter: blur(24px) saturate(1.2);
    -webkit-backdrop-filter: blur(24px) saturate(1.2);
    box-shadow:
  ...
  .tape-scroll::-webkit-scrollbar-track { background: transparent; }
  .tape-scroll::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.1); border-radius: 4px; }
  ...
  .legend-dot.training { background: var(--globe-cyan, #33d1ff); box-shadow: 0 0 8px rgba(51, 209, 255, 0.4); }
  .legend-dot.verify { background: var(--globe-orange, #ffb44d); box-shadow: 0 0 8px rgba(255, 180, 77, 0.4); }
  ...
  @media (max-width: 1280px) {
    .hud-left { width: 140px; }
    .hud-right { width: 280px; }
    .island { max-width: 700px; }
  }
  ...
  @media (max-width: 860px) {
    ...
    .island-metrics { gap: 8px; font-size: 0.7rem; }
    ...
    .stat-card { padding: 8px 10px; border-radius: 12px; }
